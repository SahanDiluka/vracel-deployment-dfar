  ...
  const handleFile = async (e) => {
    ...
    try {
      const result = type === "pdf"
        ? await officerService.uploadPdf(file)
        : await officerService.uploadImage(file);
...
function LinkTableSection({ toast }) {
  ...
  const reload = () => {
    setLoading(true);
    officerService.getAllOfficerLinkTablePages("en")
      .then(setPages)
    ...
    <div>
      ...
      {showNewPage && (
        <LinkTablePageForm
          langTab={langTab}
          onSave={async (form) => {
            try { await officerService.createOfficerLinkTablePage(form); toast.success("Page created."); setShowNewPage(false); reload(); }
          ...
          {editingPage?.id === page.id && (
            <div style={{ padding: "0 20px 16px" }}>
              <LinkTablePageForm
                ...
                onSave={async (form) => {
                  try { await officerService.updateOfficerLinkTablePage(page.id, form); toast.success("Page updated."); setEditingPage(null); reload(); }
                  catch (e) { toast.error(e.message); }
                }}
            ...
            <div style={{ borderTop: "1px solid #334155", padding: "16px 20px" }}>
                              ...
                              <RowForm
                                ...
                                onSave={async (form) => {
                                  try {
                                    ...
                                    await officerService.updateOfficerLinkTableRow(row.id, d);
              ...
              {showNewRow === page.id ? (
                <RowForm
                  ...
                  initial={null}
                  onSave={async (form) => {
                    try { await officerService.createOfficerLinkTableRow(form); toast.success("Row added."); setShowNewRow(null); reload(); }
                    catch (e) { toast.error(e.message); }
                  }}
      ...
      {deletingPage && (
        <ConfirmDelete label={deletingPage.navLabelEn}
          onConfirm={async () => {
            try { await officerService.deleteOfficerLinkTablePage(deletingPage.id); toast.success("Page deleted."); setDeletingPage(null); reload(); }
      ...
      {deletingRow && (
        <ConfirmDelete label={deletingRow.nameEn}
          onConfirm={async () => {
            try { await officerService.deleteOfficerLinkTableRow(deletingRow.id); toast.success("Row deleted."); setDeletingRow(null); reload(); }
...
function NormalPagesSection({ toast }) {
  ...
  const reload = () => {
    setLoading(true);
    officerService.getAllOfficerNormalPages("en")
      .then(setPages)
      .catch(e => { setError(e.message); toast.error(e.message); })
      .finally(() => setLoading(false));
  };
    ...
    <div>
      ...
      {showNew && (
        <NormalPageForm
          ...
          toast={toast}
          onSave={async (form) => {
            try { await officerService.createOfficerNormalPage(form); toast.success("Page created."); setShowNew(false); reload(); }
            catch (e) { toast.error(e.message); }
          }}
                    ...
                    <tr>
                      <td colSpan={4} style={{ padding: "12px 16px", background: "#172033" }}>
                        <NormalPageForm
                          ...
                          onSave={async (form) => {
                            try { await officerService.updateOfficerNormalPage(page.id, form); toast.success("Page updated."); setEditing(null); reload(); }
                            catch (e) { toast.error(e.message); }
                          }}
      ...
      {deleting && (
        <ConfirmDelete label={deleting.navLabelEn}
          onConfirm={async () => {
            try { await officerService.deleteOfficerNormalPage(deleting.id); toast.success("Page deleted."); setDeleting(null); reload(); }
...
function DirectLinkSection({ toast }) {
  ...
  const reload = () => {
    setLoading(true);
    officerService.getAllOfficerDirectLinkPages("en")
      .then(setPages)
      .catch(e => { setError(e.message); toast.error(e.message); })
      .finally(() => setLoading(false));
  };
    ...
    <div>
      ...
      {showNew && (
        <DirectLinkPageForm
          langTab={langTab}
          onSave={async (form) => {
            try { await officerService.createOfficerDirectLinkPage(form); toast.success("Link created."); setShowNew(false); reload(); }
            catch (e) { toast.error(e.message); }
          }}
          onCancel={() => setShowNew(false)}
        />
                    ...
                    <tr>
                      <td colSpan={4} style={{ padding: "12px 16px", background: "#172033" }}>
                        <DirectLinkPageForm
                          ...
                          onSave={async (form) => {
                            try { await officerService.updateOfficerDirectLinkPage(page.id, form); toast.success("Link updated."); setEditing(null); reload(); }
                            catch (e) { toast.error(e.message); }
                          }}
                          onCancel={() => setEditing(null)}
                        />
      ...
      {deleting && (
        <ConfirmDelete label={deleting.navLabelEn}
          onConfirm={async () => {
            try { await officerService.deleteOfficerDirectLinkPage(deleting.id); toast.success("Link deleted."); setDeleting(null); reload(); }
            catch (e) { toast.error(e.message); setDeleting(null); }
          }}
          onCancel={() => setDeleting(null)}
        />
    ...
    <div style={{ fontFamily: "'Inter', sans-serif", background: "#0f172a", minHeight: "100vh", padding: "32px 24px" }} className="mt-10 rounded-lg">
      ...
      <div style={{ maxWidth: 980, margin: "0 auto" }}>
        <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 28 }}>
          ...
          <h2 style={{ fontSize: 18, fontWeight: 600, color: "#f1f5f9", margin: 0 }}>Officer Pages Management</h2>
        </div>
