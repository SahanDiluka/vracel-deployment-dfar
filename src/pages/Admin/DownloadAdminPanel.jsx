...
const textareaStyle = { ...inputStyle, height: "200px", padding: "10px 12px", resize: "vertical", minHeight: 70 };
  ...
  const handleFile = async (e) => {
    ...
    try {
      const result = type === "pdf"
        ? await downloadService.uploadPdf(file)
        : await downloadService.uploadImage(file);
...
function LinkTableSection({ toast }) {
  ...
  const reload = () => {
    setLoading(true);
    downloadService.getAllDownloadLinkTablePages("en")
      .then(setPages)
    ...
    <div>
      ...
      {showNewPage && (
        <LinkTablePageForm
          langTab={langTab}
          onSave={async (form) => {
            try { await downloadService.createDownloadLinkTablePage(form); toast.success("Page created."); setShowNewPage(false); reload(); }
          ...
          {editingPage?.id === page.id && (
            <div style={{ padding: "0 20px 16px" }}>
              <LinkTablePageForm
                ...
                onSave={async (form) => {
                  try { await downloadService.updateDownloadLinkTablePage(page.id, form); toast.success("Page updated."); setEditingPage(null); reload(); }
                  catch (e) { toast.error(e.message); }
                }}
            ...
            <div style={{ borderTop: "1px solid #334155", padding: "16px 20px" }}>
                              ...
                              <RowForm
                                ...
                                onSave={async (form) => {
                                  try {
                                    ...
                                    await downloadService.updateDownloadLinkTableRow(row.id, d);
              ...
              {showNewRow === page.id ? (
                <RowForm
                  ...
                  initial={null}
                  onSave={async (form) => {
                    try { await downloadService.createDownloadLinkTableRow(form); toast.success("Row added."); setShowNewRow(null); reload(); }
                    catch (e) { toast.error(e.message); }
                  }}
      ...
      {deletingPage && (
        <ConfirmDelete label={deletingPage.navLabelEn}
          onConfirm={async () => {
            try { await downloadService.deleteDownloadLinkTablePage(deletingPage.id); toast.success("Page deleted."); setDeletingPage(null); reload(); }
      ...
      {deletingRow && (
        <ConfirmDelete label={deletingRow.nameEn}
          onConfirm={async () => {
            try { await downloadService.deleteDownloadLinkTableRow(deletingRow.id); toast.success("Row deleted."); setDeletingRow(null); reload(); }
...
function NormalPagesSection({ toast }) {
  ...
  const reload = () => {
    setLoading(true);
    downloadService.getAllDownloadNormalPages("en")
      .then(setPages)
      .catch(e => { setError(e.message); toast.error(e.message); })
      .finally(() => setLoading(false));
  };
    ...
    <div>
      ...
      {showNew && (
        <NormalPageForm
          ...
          toast={toast}
          onSave={async (form) => {
            try { await downloadService.createDownloadNormalPage(form); toast.success("Page created."); setShowNew(false); reload(); }
            catch (e) { toast.error(e.message); }
          }}
                    ...
                    <tr>
                      <td colSpan={4} style={{ padding: "12px 16px", background: "#172033" }}>
                        <NormalPageForm
                          ...
                          onSave={async (form) => {
                            try { await downloadService.updateDownloadNormalPage(page.id, form); toast.success("Page updated."); setEditing(null); reload(); }
                            catch (e) { toast.error(e.message); }
                          }}
      ...
      {deleting && (
        <ConfirmDelete label={deleting.navLabelEn}
          onConfirm={async () => {
            try { await downloadService.deleteDownloadNormalPage(deleting.id); toast.success("Page deleted."); setDeleting(null); reload(); }
...
function DirectLinkSection({ toast }) {
  ...
  const reload = () => {
    setLoading(true);
    downloadService.getAllDownloadDirectLinkPages("en")
      .then(setPages)
      .catch(e => { setError(e.message); toast.error(e.message); })
      .finally(() => setLoading(false));
  };
    ...
    <div>
      ...
      {showNew && (
        <DirectLinkPageForm
          langTab={langTab}
          onSave={async (form) => {
            try { await downloadService.createDownloadDirectLinkPage(form); toast.success("Link created."); setShowNew(false); reload(); }
            catch (e) { toast.error(e.message); }
          }}
          onCancel={() => setShowNew(false)}
        />
                    ...
                    <tr>
                      <td colSpan={4} style={{ padding: "12px 16px", background: "#172033" }}>
                        <DirectLinkPageForm
                          ...
                          onSave={async (form) => {
                            try { await downloadService.updateDownloadDirectLinkPage(page.id, form); toast.success("Link updated."); setEditing(null); reload(); }
                            catch (e) { toast.error(e.message); }
                          }}
                          onCancel={() => setEditing(null)}
                        />
      ...
      {deleting && (
        <ConfirmDelete label={deleting.navLabelEn}
          onConfirm={async () => {
            try { await downloadService.deleteDownloadDirectLinkPage(deleting.id); toast.success("Link deleted."); setDeleting(null); reload(); }
            catch (e) { toast.error(e.message); setDeleting(null); }
          }}
          onCancel={() => setDeleting(null)}
        />
...
export default function DownloadAdminPanel() {
    ...
    <div style={{ fontFamily: "'Inter', sans-serif", background: "#0f172a", minHeight: "100vh", padding: "32px 24px" }} className="mt-10 rounded-lg">
      ...
      <div style={{ maxWidth: 980, margin: "0 auto" }}>
        <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 28 }}>
          ...
          <h2 style={{ fontSize: 18, fontWeight: 600, color: "#f1f5f9", margin: 0 }}>Download Pages Management</h2>
        </div>
