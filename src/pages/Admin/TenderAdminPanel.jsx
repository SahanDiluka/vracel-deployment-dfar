...
const textareaStyle = { ...inputStyle, height: "auto", padding: "100px 12px", resize: "vertical", minHeight: 70 };
...
function Tab({ label, active, onClick }) {
  return (
    <button onClick={onClick} style={{ padding: "8px 18px", fontSize: 13, fontWeight: 600, borderRadius: 8, border: "none", cursor: "pointer", transition: "all 0.15s", background: active ? "#2563eb" : "transparent", color: active ? "#fff" : "#64748b" }}>
      {label}
    </button>
  );
}
...
function FileUploadButton({ label, accept, onUploaded, onError, type = "pdf" }) {
  ...
  const handleFile = async (e) => {
    ...
    try {
      const result = type === "pdf"
        ? await tenderService.uploadPdf(file)
        : await tenderService.uploadImage(file);
  ...
  return (
    <>
      ...
      <button type="button" onClick={() => ref.current?.click()} disabled={loading} style={{ display: "inline-flex", alignItems: "center", gap: 6, height: 34, padding: "0 14px", fontSize: 12, fontWeight: 500, color: "#60a5fa", background: "#172033", border: "1px solid #1e3a5f", borderRadius: 8, cursor: "pointer" }}>
  ...
  return (
    <div style={{ background: "#1e293b", border: "1px solid #334155", borderRadius: 12, overflow: "hidden" }}>
      <div style={{ padding: "12px 20px", borderBottom: "1px solid #334155" }}>
        <p style={{ fontSize: 13, fontWeight: 500, color: "#64748b", margin: 0 }}>{count} {noun}{count !== 1 ? "s" : ""}</p>
      </div>
      {children}
    </div>
  );
...
function Td({ children, mono = false }) {
  return <td style={{ padding: "10px 16px", color: "#94a3b8", fontFamily: mono ? "monospace" : "inherit", fontSize: mono ? 12 : 13 }}>{children}</td>;
}
...
// ══════════════════════════════════════════════════════════════════════════════
// ROW FORM — top-level: prevents state loss on parent re-render
// ══════════════════════════════════════════════════════════════════════════════
    ...
    <div style={{ background: "#172033", border: "1px solid #1e3a5f", borderRadius: 10, padding: "16px 20px", marginBottom: 12 }}>
      ...
      <form onSubmit={async e => { e.preventDefault(); setSaving(true); try { await onSave(form); } finally { setSaving(false); } }}>
        <div style={{ marginBottom: 12 }}>
          {langTab === "en" && <><FieldLabel text="Name (EN)" /><input style={inputStyle} value={form.name_en} onChange={e => set("name_en", e.target.value)} required /></>}
          {langTab === "si" && <><FieldLabel text="Name (SI)" /><input style={inputStyle} value={form.name_si} onChange={e => set("name_si", e.target.value)} /></>}
          {langTab === "ta" && <><FieldLabel text="Name (TA)" /><input style={inputStyle} value={form.name_ta} onChange={e => set("name_ta", e.target.value)} /></>}
        </div>
        <div style={{ marginBottom: 12 }}>
          <FieldLabel text="URL (leave blank if uploading PDF)" />
          <input style={inputStyle} value={form.url} placeholder="https://..." onChange={e => { set("url", e.target.value); if (e.target.value) set("pdf_path", ""); }} />
        </div>
        <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 12 }}>
          <span style={{ fontSize: 11, color: "#475569" }}>or</span>
          <FileUploadButton
            label="Upload PDF" accept=".pdf,application/pdf" type="pdf"
            onUploaded={(path) => {
          ...
          {form.pdf_path && (
            <span style={{ fontSize: 11, color: "#4ade80", display: "flex", alignItems: "center", gap: 4 }}>
              ...
              <button type="button" onClick={() => set("pdf_path", "")} style={{ background: "none", border: "none", cursor: "pointer", color: "#f87171", display: "flex", alignItems: "center", padding: 0, marginLeft: 2 }}>
                <X size={11} />
              </button>
            </span>
          )}
...
// ══════════════════════════════════════════════════════════════════════════════
// LINK TABLE PAGE FORM — top-level
// ══════════════════════════════════════════════════════════════════════════════

function LinkTablePageForm({ initial = {}, langTab, onSave, onCancel }) {
  const [form, setForm] = useState({
    ...
    <FormCard>
      ...
      <form onSubmit={async e => { e.preventDefault(); setSaving(true); try { await onSave(form); } finally { setSaving(false); } }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 120px 120px", gap: 12, marginBottom: 16 }}>
          <div><FieldLabel text="Slug" /><input style={inputStyle} value={form.slug} onChange={e => set("slug", e.target.value)} placeholder="e.g. forms" required /></div>
...
// ══════════════════════════════════════════════════════════════════════════════
// NORMAL PAGE FORM — top-level: has image upload state that must survive re-renders
// ══════════════════════════════════════════════════════════════════════════════

function NormalPageForm({ initial = {}, langTab, onSave, onCancel, toast }) {
  const empty = {
    nav_label_en: "", nav_label_si: "", nav_label_ta: "",
    header_en: "", header_si: "", header_ta: "",
      ...
      <form onSubmit={async e => { e.preventDefault(); setSaving(true); try { await onSave(form); } finally { setSaving(false); } }}>
        {langTab === "en" && (
          <div style={{ display: "flex", flexDirection: "column", gap: 12, marginBottom: 12 }}>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
              <div><FieldLabel text="Second Header (EN)" /><input style={inputStyle} value={form.second_header_en} onChange={e => set("second_header_en", e.target.value)} /></div>
              <div><FieldLabel text="Button Label (EN)" /><input style={inputStyle} value={form.button_label_en} onChange={e => set("button_label_en", e.target.value)} /></div>
            </div>
          </div>
        )}
        {langTab === "si" && (
...
// ══════════════════════════════════════════════════════════════════════════════
// DIRECT LINK PAGE FORM — top-level
// ══════════════════════════════════════════════════════════════════════════════

function DirectLinkPageForm({ initial = {}, langTab, onSave, onCancel }) {
  const [form, setForm] = useState({
    nav_label_en: "", nav_label_si: "", nav_label_ta: "",
...
function LinkTableSection({ toast }) {
  ...
  const reload = () => {
    setLoading(true);
    tenderService.getAllTenderLinkTablePages("en")
      .then(setPages)
  ...
  return (
    <div>
      ...
      <SectionHeader icon={Table2} title="Link Table Pages" onAdd={() => { setShowNewPage(true); setEditingPage(null); }} addLabel="New Page" />

      {showNewPage && (
        <LinkTablePageForm
          langTab={langTab}
          onSave={async (form) => {
            try { await tenderService.createTenderLinkTablePage(form); toast.success("Page created."); setShowNewPage(false); reload(); }
      ...
      {pages.length === 0 && !showNewPage && (
        <p style={{ textAlign: "center", color: "#475569", fontSize: 13, padding: 32 }}>No pages yet. Create one above.</p>
      )}

      <LangTabs active={langTab} onChange={setLangTab} />

      {pages.map(page => (
        <div key={page.id} style={{ background: "#1e293b", border: "1px solid #334155", borderRadius: 12, marginBottom: 12, overflow: "hidden" }}>

          <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "14px 20px" }}>
            <button type="button" onClick={() => setExpanded(expanded === page.id ? null : page.id)}
              ...
              {expanded === page.id ? <ChevronDown size={15} color="#3b82f6" /> : <ChevronRight size={15} color="#475569" />}
              {page.navLabelEn}
              <span style={{ fontSize: 11, color: "#475569", fontWeight: 400 }}>/{page.slug}</span>
              {!page.isActive && <span style={{ fontSize: 10, background: "#1f1215", color: "#f87171", border: "1px solid #7f1d1d", borderRadius: 20, padding: "1px 8px" }}>inactive</span>}
            </button>
            <div style={{ display: "flex", gap: 6 }}>
              <button onClick={() => { setEditingPage(editingPage?.id === page.id ? null : page); setShowNewPage(false); }}
          ...
          {editingPage?.id === page.id && (
            <div style={{ padding: "0 20px 16px" }}>
              <LinkTablePageForm
                langTab={langTab}
                initial={{ nav_label_en: page.navLabelEn, nav_label_si: page.navLabelSi, nav_label_ta: page.navLabelTa, header_en: page.headerEn, header_si: page.headerSi, header_ta: page.headerTa, slug: page.slug, is_active: page.isActive, sort_order: page.sortOrder }}
                onSave={async (form) => {
                  try { await tenderService.updateTenderLinkTablePage(page.id, form); toast.success("Page updated."); setEditingPage(null); reload(); }
                  catch (e) { toast.error(e.message); }
                }}
            ...
            <div style={{ borderTop: "1px solid #334155", padding: "16px 20px" }}>
                  ...
                  {page.rows.map((row, i) => (
                    <React.Fragment key={row.id}>
                      <tr style={{ borderBottom: "1px solid #1e293b" }}>
                        ...
                        <Td>
                          {row.pdfPath
                            ? <span style={{ display: "flex", alignItems: "center", gap: 5, color: "#4ade80" }}><FileText size={11} /> PDF</span>
                            : <span style={{ display: "flex", alignItems: "center", gap: 5 }}><LinkIcon size={11} /><span style={{ maxWidth: 200, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap", display: "block" }}>{row.url}</span></span>}
                        </Td>
                        <ActionButtons
                          ...
                          <td colSpan={4} style={{ padding: 0, background: "#0f1f33" }}>
                            <div style={{ padding: "12px 16px" }}>
                              <RowForm
                                ...
                                pageId={page.id}
                                toast={toast}
                                initial={{ name_en: row.nameEn, name_si: row.nameSi ?? "", name_ta: row.nameTa ?? "", url: row.url ?? "", pdf_path: row.pdfPath ?? "", sort_order: row.sortOrder }}
                                onSave={async (form) => {
                                  try {
                                    const { page_id, ...d } = form;
                                    await tenderService.updateTenderLinkTableRow(row.id, d);
                                    toast.success("Row updated."); setEditingRow(null); reload();
                                  } catch (e) { toast.error(e.message); }
                                }}
              ...
              {showNewRow === page.id ? (
                <RowForm
                  ...
                  initial={null}
                  onSave={async (form) => {
                    try { await tenderService.createTenderLinkTableRow(form); toast.success("Row added."); setShowNewRow(null); reload(); }
                    catch (e) { toast.error(e.message); }
                  }}
                  onCancel={() => setShowNewRow(null)}
                />
              ) : (
                <button onClick={() => setShowNewRow(page.id)} style={{ display: "flex", alignItems: "center", gap: 6, fontSize: 12, fontWeight: 600, color: "#3b82f6", background: "none", border: "none", cursor: "pointer" }}>
      ...
      {deletingPage && (
        <ConfirmDelete label={deletingPage.navLabelEn}
          onConfirm={async () => {
            try { await tenderService.deleteTenderLinkTablePage(deletingPage.id); toast.success("Page deleted."); setDeletingPage(null); reload(); }
      ...
      {deletingRow && (
        <ConfirmDelete label={deletingRow.nameEn}
          onConfirm={async () => {
            try { await tenderService.deleteTenderLinkTableRow(deletingRow.id); toast.success("Row deleted."); setDeletingRow(null); reload(); }
...
function NormalPagesSection({ toast }) {
  ...
  const reload = () => {
    setLoading(true);
    tenderService.getAllTenderNormalPages("en")
      .then(setPages)
      .catch(e => { setError(e.message); toast.error(e.message); })
      .finally(() => setLoading(false));
  };
  ...
  return (
    <div>
      <ErrorBox message={error} />
      <SectionHeader icon={AlignLeft} title="Normal Pages" onAdd={() => { setShowNew(true); setEditing(null); }} addLabel="New Page" />
      ...
      {showNew && (
        <NormalPageForm
          ...
          toast={toast}
          onSave={async (form) => {
            try { await tenderService.createTenderNormalPage(form); toast.success("Page created."); setShowNew(false); reload(); }
            catch (e) { toast.error(e.message); }
          }}
      ...
      {pages.length === 0 && !showNew && <p style={{ textAlign: "center", color: "#475569", fontSize: 13, padding: 32 }}>No pages yet.</p>}
      ...
      <TableCard count={pages.length} noun="page">
        {pages.length > 0 && (
          <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 13 }}>
            <thead><tr style={{ background: "#0f172a" }}><Th>Nav Label</Th><Th>Slug</Th><Th>Status</Th><Th>Actions</Th></tr></thead>
            <tbody>
              {pages.map(page => (
                <React.Fragment key={page.id}>
                  <tr style={{ borderBottom: "1px solid #1e293b", background: editing?.id === page.id ? "#172033" : "transparent" }}>
                    ...
                    <Td>/{page.slug}</Td>
                    <td style={{ padding: "10px 16px" }}>
                      <span style={{ display: "inline-block", padding: "2px 10px", borderRadius: 20, fontSize: 11, fontWeight: 600, background: page.isActive ? "#1a2e1a" : "#1f1215", color: page.isActive ? "#4ade80" : "#f87171", border: `1px solid ${page.isActive ? "#14532d" : "#7f1d1d"}` }}>
                    ...
                    <tr>
                      <td colSpan={4} style={{ padding: "12px 16px", background: "#172033" }}>
                        <NormalPageForm
                          langTab={langTab}
                          toast={toast}
                          initial={{ nav_label_en: page.navLabelEn, nav_label_si: page.navLabelSi, nav_label_ta: page.navLabelTa, header_en: page.headerEn, header_si: page.headerSi, header_ta: page.headerTa, description_en: page.descriptionEn, description_si: page.descriptionSi, description_ta: page.descriptionTa, second_header_en: page.secondHeaderEn, second_header_si: page.secondHeaderSi, second_header_ta: page.secondHeaderTa, button_label_en: page.buttonLabelEn, button_label_si: page.buttonLabelSi, button_label_ta: page.buttonLabelTa, button_url: page.buttonUrl, image_path: page.imagePath, slug: page.slug, is_active: page.isActive, sort_order: page.sortOrder }}
                          onSave={async (form) => {
                            try { await tenderService.updateTenderNormalPage(page.id, form); toast.success("Page updated."); setEditing(null); reload(); }
                            catch (e) { toast.error(e.message); }
                          }}
      ...
      {deleting && (
        <ConfirmDelete label={deleting.navLabelEn}
          onConfirm={async () => {
            try { await tenderService.deleteTenderNormalPage(deleting.id); toast.success("Page deleted."); setDeleting(null); reload(); }
...
function DirectLinkSection({ toast }) {
  ...
  const reload = () => {
    setLoading(true);
    tenderService.getAllTenderDirectLinkPages("en")
      .then(setPages)
      .catch(e => { setError(e.message); toast.error(e.message); })
      .finally(() => setLoading(false));
  };
  ...
  return (
    <div>
      <ErrorBox message={error} />
      <SectionHeader icon={LinkIcon} title="Direct Link Pages" onAdd={() => { setShowNew(true); setEditing(null); }} addLabel="New Link" />

      {showNew && (
        <DirectLinkPageForm
          langTab={langTab}
          onSave={async (form) => {
            try { await tenderService.createTenderDirectLinkPage(form); toast.success("Link created."); setShowNew(false); reload(); }
            catch (e) { toast.error(e.message); }
          }}
          onCancel={() => setShowNew(false)}
        />
      ...
      {pages.length === 0 && !showNew && <p style={{ textAlign: "center", color: "#475569", fontSize: 13, padding: 32 }}>No direct links yet.</p>}
      ...
      <TableCard count={pages.length} noun="link">
        {pages.length > 0 && (
          <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 13 }}>
            <thead><tr style={{ background: "#0f172a" }}><Th>Nav Label</Th><Th>URL</Th><Th>Status</Th><Th>Actions</Th></tr></thead>
            <tbody>
              {pages.map(page => (
                <React.Fragment key={page.id}>
                  <tr style={{ borderBottom: "1px solid #1e293b", background: editing?.id === page.id ? "#172033" : "transparent" }}>
                    ...
                    <td style={{ padding: "10px 16px", maxWidth: 300 }}>
                      <span style={{ color: "#3b82f6", fontSize: 12, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap", display: "block" }}>{page.url}</span>
                    </td>
                    <td style={{ padding: "10px 16px" }}>
                      <span style={{ display: "inline-block", padding: "2px 10px", borderRadius: 20, fontSize: 11, fontWeight: 600, background: page.isActive ? "#1a2e1a" : "#1f1215", color: page.isActive ? "#4ade80" : "#f87171", border: `1px solid ${page.isActive ? "#14532d" : "#7f1d1d"}` }}>
                        {page.isActive ? "Active" : "Inactive"}
                  ...
                  {editing?.id === page.id && (
                    <tr>
                      <td colSpan={4} style={{ padding: "12px 16px", background: "#172033" }}>
                        <DirectLinkPageForm
                          langTab={langTab}
                          initial={{ nav_label_en: page.navLabelEn, nav_label_si: page.navLabelSi, nav_label_ta: page.navLabelTa, url: page.url, is_active: page.isActive, sort_order: page.sortOrder }}
                          onSave={async (form) => {
                            try { await tenderService.updateTenderDirectLinkPage(page.id, form); toast.success("Link updated."); setEditing(null); reload(); }
                            catch (e) { toast.error(e.message); }
                          }}
                          onCancel={() => setEditing(null)}
                        />
      ...
      {deleting && (
        <ConfirmDelete label={deleting.navLabelEn}
          onConfirm={async () => {
            try { await tenderService.deleteTenderDirectLinkPage(deleting.id); toast.success("Link deleted."); setDeleting(null); reload(); }
            catch (e) { toast.error(e.message); setDeleting(null); }
          }}
          onCancel={() => setDeleting(null)}
        />
...
export default function TenderAdminPanel() {
    ...
    <div style={{ fontFamily: "'Inter', sans-serif", background: "#0f172a", minHeight: "100vh", padding: "32px 24px" }} className="mt-10 rounded-lg">
      ...
      <div style={{ maxWidth: 980, margin: "0 auto" }}>
        <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 28 }}>
          ...
          <h2 style={{ fontSize: 18, fontWeight: 600, color: "#f1f5f9", margin: 0 }}>Tender Pages Management</h2>
        </div>

        <div style={{ display: "flex", gap: 4, background: "#1e293b", borderRadius: 10, padding: 4, marginBottom: 28, width: "fit-content" }}>
          {TABS.map(tab => <Tab key={tab.id} label={tab.label} active={activeTab === tab.id} onClick={() => setActiveTab(tab.id)} />)}
        </div>
