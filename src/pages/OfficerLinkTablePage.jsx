...
import { useOfficerLinkTablePage } from "../hooks/officer_hook";
import { FileText, ExternalLink, Download } from "lucide-react";
import { useSelector } from "react-redux";
import { selectLanguage } from "../languageSlice";
import Breadcrumb from "../components/Breadcrumb";
export default function OfficerLinkTablePage() {
  const { slug } = useParams();
  const { page, loading, error } = useOfficerLinkTablePage(slug);
              ...
              <div className="bg-white rounded-2xl shadow-[0_2px_24px_rgba(0,0,0,0.07)] border border-gray-100 overflow-hidden fade-up">
                <table className="w-full text-sm">
                  ...
                  <tbody>
                    {page.rows.length === 0 && (
                      <tr>
                        <td colSpan={3} className="px-6 py-10 text-center text-gray-400">No items yet.</td>
                      </tr>
                    )}
                    {page.rows.map((row, i) => {
